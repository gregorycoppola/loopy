...
def run(n_trials: int = 100):
    console.print("[bold cyan]Square Graph Experiment[/bold cyan]")
    console.print(f"Running {n_trials} trials with random factor tables.\n")

    results = []
    for seed in range(n_trials):
        g = random_square(seed=seed)
    ...
    table = Table(title="Square Results")
